...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build where clause
    const where: Record<string, unknown> = { assetCategory: 'MOTOR_SLEEVE' };
    ...
    console.error('[GET /api/motor-sleeves]', error);
    return NextResponse.json(
      { error: 'An unexpected error occurred while fetching motor sleeves.' },
      { status: 500 },
    );
  }
}
