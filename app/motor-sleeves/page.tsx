...
export const metadata = {
  title: 'Motor Sleeves — Motor Hour Tracker',
  description: 'Browse all motor sleeve sub-components by serial number, status, and pumping hours.',
};
...
const BASE = '/motor-sleeves';
const CATEGORY = 'MOTOR_SLEEVE';

export default async function MotorSleevesPage({
  searchParams,
}: {
  ...
  const { items: sleeves, total } = await listSubComponents({ ...params, assetCategory: CATEGORY });
  ...
  return (
    <div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 md:mb-8 animate-fade-in">
        <div>
          <h1 className="text-2xl font-bold text-[#121212] tracking-tight">Motor Sleeves</h1>
          <p className="text-sm text-[#333333] mt-1">
            {total} sleeve{total !== 1 ? 's' : ''} registered
          </p>
        </div>
        <Link
      ...
      {sleeves.length > 0 ? (
        ...
        <div className="text-center py-20 animate-fade-in">
          ...
          <p className="text-[#333333] text-sm mb-2">
            {hasFilters ? 'No motor sleeves match your filters.' : 'No motor sleeves registered.'}
          </p>
          {hasFilters && (
            <Link
              href={BASE}
