...
export const metadata = {
  title: 'Rotors — Motor Hour Tracker',
  description: 'Browse all rotor sub-components by serial number, status, and pumping hours.',
};
...
const BASE = '/rotors';
const CATEGORY = 'ROTOR';

export default async function RotorsPage({
  searchParams,
}: {
  ...
  const { items: rotors, total } = await listSubComponents({ ...params, assetCategory: CATEGORY });
  ...
  return (
    <div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 md:mb-8 animate-fade-in">
        <div>
          <h1 className="text-2xl font-bold text-[#121212] tracking-tight">Rotors</h1>
          <p className="text-sm text-[#333333] mt-1">
            {total} rotor{total !== 1 ? 's' : ''} registered
          </p>
        </div>
        <Link
      ...
      {rotors.length > 0 ? (
        ...
        <div className="text-center py-20 animate-fade-in">
          ...
          <p className="text-[#333333] text-sm mb-2">
            {hasFilters ? 'No rotors match your filters.' : 'No rotors registered.'}
          </p>
          {hasFilters && (
            <Link
              href={BASE}
